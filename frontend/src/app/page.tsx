...
import { 
  ...
  ArrowRightCircle, Layers, Clock, Smartphone, Grid
} from "lucide-react";
...
export default function Home() {
  ...
  // App State - Adicionei 'gallery' como um modo
  ...
  const fetchHistory = async (userId: string) => {
      const { data } = await supabase
        ...
        .limit(20); // Aumentei o limite para o Dashboard
  ...
  const handleGenerate = async () => {
    ...
    if (isEditingContext) {
        const engineeredPrompt = `INSTRUCTION: EDIT THE ATTACHED IMAGE. Keep the composition... Change: ${prompt}`;
    ...
    try {
      ...
      setPendingResult(url); // Apenas guarda, a lógica do useEffect vai decidir quando fechar
  ...
  // --- LÓGICA DE SINCRONIA DO ANÚNCIO (CORRIGIDA) ---
  useEffect(() => {
      // Se for VÍDEO e já tiver resultado, fecha na hora (não espera o loop acabar)
  ...
  const handleAdEnded = () => {
    // Se for IMAGEM e já tiver resultado, fecha quando o vídeo acabar
    if (mode === "image" && pendingResult) {
        setResultUrl(pendingResult);
        setLoading(false);
        setPendingResult(null);
    } 
    // Se não tiver resultado ainda, o componente AdPlayer fará o loop sozinho?
    // O componente AdPlayer não tem loop automático, então precisamos reiniciar aqui se ainda estiver carregando
    // Mas no modo video o useEffect acima já resolveu.
  };
    ...
    <main className="min-h-screen bg-[#050505] text-white flex flex-col font-sans relative overflow-x-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 py-10 w-full max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex w-full bg-gray-900 p-1.5 rounded-2xl border border-gray-800">
            ...
            <button onClick={() => {setMode("gallery");}} className={`flex-1 py-3 rounded-xl flex items-center justify-center gap-2 font-bold transition-all ${mode === "gallery" ? "bg-yellow-900/30 text-yellow-200 shadow-lg border border-yellow-800/50" : "text-gray-500 hover:text-gray-300"}`}><Grid className="w-5 h-5" /> Galeria</button>
        </div>

        {/* --- TELA DE CRIAÇÃO (IMAGEM E VÍDEO) --- */}
        {mode !== "gallery" && (
            <>
                <div className="w-full bg-[#0f0f10] border border-gray-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="space-y-4 mb-6">
                        ...
                        {isEditing && (
                            <div className="flex items-center gap-2 text-xs text-yellow-500 bg-yellow-500/10 p-2 rounded-lg border border-yellow-500/20"><Layers className="w-4 h-4" /><span>Modo Edição Ativo: A IA vai alterar a imagem atual.</span><button onClick={handleClearAll} className="ml-auto hover:underline text-gray-400 hover:text-white">Limpar</button></div>
                        )}
        ...
        {/* --- TELA DA GALERIA (Item 3 - Dashboard) --- */}
        {mode === "gallery" && (
            <div className="w-full bg-[#0f0f10] border border-gray-800 rounded-3xl p-6 shadow-2xl animate-in fade-in">
                <h3 className="text-white font-bold text-xl mb-6 flex items-center gap-2 border-b border-gray-800 pb-4">
                    <Clock className="w-6 h-6 text-yellow-500" /> Minha Galeria
                </h3>
                {history.length === 0 ? (
                    <div className="text-center py-20 text-gray-500">
                        <p>Você ainda não criou nada.</p>
                        <button onClick={() => setMode("image")} className="mt-4 text-yellow-500 hover:underline">Começar agora</button>
                    </div>
                ) : (
