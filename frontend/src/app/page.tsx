...
import {
    ...
    X, MessageSquare, Send, Bot, Zap, PlayCircle, Eye, LogOut,
    XCircle
} from "lucide-react";
...
// PERSONAS (Nah configurada como IA Geral)
const PERSONAS = [
...
export default function Home() {
        ...
        try {
            if (mode === "image") {
                if (imageFiles.length > 0) imageFiles.forEach(file => formData.append("files", file));
                    ...
                    try { const res = await fetch(previousResult); const blob = await res.blob(); const file = new File([blob], "context.jpg", { type: "image/jpeg" }); formData.append("files", file); } catch (e) { }
                }
    ...
    const currentCost = mode === "image" ? (imageFiles.length > 1 || isEditing ? 10 : 10) : 50;

    return (
        // FIX: h-screen e overflow-hidden para evitar rolagem da página inteira no mobile
        <main className="h-screen bg-[#050505] text-white flex flex-col font-sans relative overflow-hidden">

            <header className="w-full p-4 border-b border-gray-800 bg-black/50 backdrop-blur-md flex justify-between items-center z-30 shrink-0">
                            ...
                            <div className="fixed top-16 right-4 z-50 w-72 bg-[#18181b] border border-gray-700 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2" onClick={(e) => e.stopPropagation()}>
                                ...
                                <div className="max-h-60 overflow-y-auto">
                                    {notifications.length === 0 ? (
                                        <div className="p-4 text-center text-xs text-gray-600">Nenhuma notificação nova.</div>
                                    ) : (
                                        notifications.map(n => (
            ...
            <div className="flex-1 overflow-y-auto w-full">
                <div className="flex flex-col items-center justify-center p-4 py-6 w-full max-w-5xl mx-auto space-y-6">

                    {mode === 'home' && (
                        <div className="w-full animate-in fade-in slide-in-from-bottom-4 space-y-6 pb-20">
                            ...
                            <div className="space-y-2">
                                ...
                                <button onClick={() => setMode('chat')} className="w-full bg-[#18181b] hover:bg-[#202024] p-4 rounded-2xl flex items-center gap-4 border border-gray-800 transition-all group">
                                    ...
                                    <div className="text-left flex-1">
                                        <h4 className="font-bold text-white">Chat & IA (Nah)</h4>
                                        <p className="text-xs text-gray-400">Converse, crie textos e tire dúvidas.</p>
                                    </div>
                    ...
                    {mode !== 'home' && (
                        <div className="flex w-full bg-gray-900 p-1.5 rounded-2xl border border-gray-800 overflow-x-auto shrink-0 mb-4">
                    ...
                    {mode === "chat" && (
                        <div className="w-full flex flex-col md:flex-row gap-4 h-[60vh] md:h-[600px]">
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                    {chatHistory.length === 0 && (
                                        <div className="h-full flex flex-col items-center justify-center text-gray-600 text-center p-6">
                                            ...
                                            <p className="text-sm font-bold">Olá! Sou a {currentPersona.name}.</p>
                                            <p className="text-xs mt-1 max-w-xs">{currentPersona.prompt.split('.')[1]}</p>
                                        </div>
                                    )}
                    ...
                    {(mode === "image" || mode === "video") && (
                        <div className="w-full bg-[#0f0f10] border border-gray-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden group pb-20">
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {imageFiles.map((file, idx) => (
                                        <div key={idx} className="relative w-20 h-20 bg-gray-800 rounded-xl overflow-hidden border border-gray-700 group/img">
                                            {/* CORREÇÃO: Adicionado alt="preview" que é obrigatório */}
                                            ...
                                            <button onClick={() => removeImage(idx)} className="absolute top-0.5 right-0.5 bg-black/50 text-white rounded-full p-0.5 hover:bg-red-500">
                    ...
                    {mode === "gallery" && (
                        <div className="w-full bg-[#0f0f10] border border-gray-800 rounded-3xl p-6 shadow-2xl animate-in fade-in pb-20">
