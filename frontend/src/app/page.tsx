...
import {
    ...
    ArrowRightCircle, Layers, Clock, Smartphone, CheckCircle, RectangleHorizontal, RectangleVertical, Bell, ExternalLink
} from "lucide-react";
...
export default function Home() {
    ...
    const handleEditFromGallery = async (url: string) => {
        setResultUrl(url);
            ...
            <header className="w-full p-4 border-b border-gray-800 bg-black/50 backdrop-blur-md flex justify-between items-center sticky top-0 z-30">
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        <button onClick={() => setShowNotifications(!showNotifications)} className="p-2 hover:bg-gray-800 rounded-full text-gray-400 hover:text-white relative"><Bell className="w-5 h-5" />{notifications.length > 0 && <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-[#050505]"></span>}</button>
                        {showNotifications && (
                            <div className="absolute top-full right-0 mt-2 w-64 bg-[#18181b] border border-gray-700 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                                <div className="p-3 border-b border-gray-800 text-xs font-bold text-gray-400">Notificações</div>
                                    ...
                                    <div key={item.id} className="aspect-square bg-gray-900 rounded-xl overflow-hidden relative group">
                                        ...
                                        <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-2">
                                            <div className="flex gap-2">
                                                ...
                                                {/* BOTÃO DE ANIMAR E EDITAR DA GALERIA */}
                                                {item.type === 'image' && (
                                                    <>
                                                        ...
                                                        {!isMobile && <button onClick={() => handleEditFromGallery(item.url)} className="p-2 bg-yellow-500 text-black rounded-full hover:scale-110 transition-transform" title="Editar"><Edit className="w-4 h-4" /></button>}
                                                    </>
                                                )}
                                            </div>
                                        </div>
