...
export default function StoreModal({ userId, currentPlan, referralCode, onClose, onUpdate }: StoreModalProps) {
    ...
    const PRODUCTS = [
        // ASSINATURAS
        {
            ...
            credits: 100,
            price: "R$ 0",
            features: ["100 Créditos/mês", "Com Marca d'água", "Acesso Básico"]
        },
        {
            id: "plus",
            type: "plan",
            ...
            price: "R$ 69",
            sub: "/mês",
            features: ["500 Créditos/mês", "🚫 SEM Marca d'água", "Editor de Imagem"]
        },
        {
            id: "pro",
            type: "plan",
            ...
            price: "R$ 99",
            sub: "/mês",
            features: ["1000 Créditos/mês", "🚫 SEM Marca d'água", "Prioridade na Fila"]
        },
        // PACOTE AVULSO (NOVO)
        {
            id: "pack600",
            ...
            price: "R$ 99",
            sub: "/único",
            features: ["600 Créditos válidos por 1 ano", "Não renova automaticamente", "Ideal para projetos pontuais"]
        },
    ];
                            ...
                            {PRODUCTS.map((prod) => (
                                <div key={prod.id} className={`p-5 rounded-xl border relative flex flex-col ${currentPlan === prod.id ? "border-yellow-500 bg-yellow-500/5" : prod.type === 'pack' ? "border-blue-500/30 bg-blue-900/10" : "border-gray-700 bg-[#202022]"}`}>
                                    ...
                                    <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                        {prod.name}
                                        {prod.type === 'pack' && <Zap className="w-4 h-4 text-blue-400 fill-blue-400" />}
                                    </h3>

                                    <div className="text-2xl font-bold text-white my-2">
                                        {prod.price}<span className="text-sm text-gray-500 font-normal">{prod.sub}</span>
                                    </div>
                                    ...
                                    {prod.id !== 'free' && (
                                        ...
                                        >
                                            <button
                                                disabled={currentPlan === prod.id && prod.type !== 'pack'} // Permite comprar Pack mesmo se já for Pro
                                                className={`w-full py-2 rounded-lg text-sm font-bold transition-all ${currentPlan === prod.id && prod.type !== 'pack'
